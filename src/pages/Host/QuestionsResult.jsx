...
const QuestionsResult = ({ data }) => {
	...
	const handleNextQuestion = async () => {
		...
		try {
			...
			setError(
				ERROR_MESSAGES.NEXT_QUESTION_ERROR || "Failed to load next question",
			);
		} finally {
			setIsLoading(false);
		}
	...
	return (
		<div className="flex flex-col overflow-y-auto">
			<div className="flex-1 px-6 py-4 space-y-6 overflow-y-auto">
				{error && (
					<div
						className="bg-red-600 text-white p-3 rounded-full text-center"
						dir="rtl"
					>
						{error}
					</div>
				...
				<div className="text-right" dir="rtl">
					<p className="leading-relaxed text-heading">{question}</p>
				</div>
					...
					<div className="space-y-3">
						{leaderboard.map((player, index) => (
							...
							>
								...
								<span className="text-white text-primary-button" dir="ltr">
									{player.score} PT
								</span>
							</div>
						))}
