...
const HostWaitingpage = ({ leaderboard }) => {
  ...
  const storedId =
    typeof window !== "undefined" ? localStorage.getItem("game_id") : null;
  ...
  return (
    <div className="flex flex-col">
      <div className="flex-1 px-6 py-4 space-y-6 overflow-y-auto">
        {isLoading && (
          <div className="text-center py-8" dir="rtl">
            <div className="text-white text-lg mb-2">
              {ERROR_MESSAGES.LOADING_GAME_DATA}
            </div>
        ...
        {error && (
          <div
            className="bg-red-600 text-white p-4 rounded-full text-center"
            dir="rtl"
          >
            {error}
          </div>
            ...
            <div dir="rtl">
              ...
              <div className="space-y-3">
                {participants.map((participant, index) => (
                  <div
                    key={participant.id}
                    className="flex items-center justify-between py-2 border-b border-green-400"
                  >
                    <div className="flex items-center space-x-3" dir="rtl">
                      ...
                      <span className="text-white text-primary-button">
                        {participant.name}
                      </span>
                    </div>
                  </div>
              ...
              {participants.length < 1 && (
                <div className="text-center py-4 text-white font-saudi text-sm">
                  {ERROR_MESSAGES.WAITING_FOR_PLAYERS}
                </div>
              )}
            </div>
          </>
      ...
      {!isLoading && !error && data && (
        <div className="p-6 flex flex-col items-center">
          <GreenButton
            text={isFinished ? UI_TEXT.GAME_FINISHED : UI_TEXT.START_BUTTON}
            handleClick={handleStartQuiz}
            disabled={data === null || data.leaderboard.length < 2}
          />
          {totalPlayers < 1 && !isFinished && (
            <p
              className="text-center text-white font-saudi text-sm mt-2"
              dir="rtl"
            >
